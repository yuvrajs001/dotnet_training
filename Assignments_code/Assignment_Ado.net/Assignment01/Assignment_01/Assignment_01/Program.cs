...
{ 
       class Employee
       {
        ...
        class Program
        {
            ...
            var Before_Join = empTable.AsEnumerable().Where(row => row.Field<DateTime>("DOJ") < new DateTime(2015, 1, 1));
                
                foreach (var employee in Before_Join)
                {
            ...
            int After_join = empTable.AsEnumerable().Count(row => row.Field<DateTime>("DOJ") < new DateTime(2015, 1, 1));
                Console.WriteLine($"\n7.Total number of joined after 1/1/2015 : {After_join}");
